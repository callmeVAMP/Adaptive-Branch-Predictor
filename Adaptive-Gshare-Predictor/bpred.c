...
/* ===========================================================
 * Adaptive Gshare Helper Functions
 * =========================================================== */

static inline void sat_inc(unsigned char *x) {
  if (*x < 3) (*x)++;
}

static inline void sat_dec(unsigned char *x) {
  if (*x > 0) (*x)--;
}
...
case BPredAdaptiveGshare:   /* ✅ include our new predictor */
    {
      int i;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  fprintf(stderr, "[bpred_dir_create] class=%d l1=%u l2=%u shift=%u xor=%u\n",
        class, l1size, l2size, shift_width, xor);
					 ...
					 * used on mispredict recovery */
{
  ...
  unsigned int rotated_hist = ((long_hist << 2) | (long_hist >> (long_w - 2))) & ((1 << long_w) - 1);
  long_idx = (( (baddr >> 2) ^ rotated_hist ^ (baddr & 0x3F) )) & ((1 << long_w) - 1);
    ...
    dir_update_ptr->pdir1 = (char *)&pred->adaptive.gshare_short->config.two.l2table[short_idx];
    dir_update_ptr->pdir2 = (char *)&pred->adaptive.gshare_long->config.two.l2table[long_idx];
    dir_update_ptr->pmeta = (char *)&pred->adaptive.chooser->config.bimod.table[chooser_idx];
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (!!pred_taken == !!taken)
    ...
    pred->misses++;
  /* ======================================================
   * Adaptive Gshare update (only if this is our predictor)
   * ====================================================== */
...
if (short_pred != long_pred) {
  ...
  /*unsigned char *short_ctr   = (unsigned char *)dir_update_ptr->pdir1;
    ...
    /* === 1. Update predictors (only if mispredicted) === 
    if (short_pred != taken) {
        if (taken) {
            if (*short_ctr < 3) (*short_ctr)++;
        } else {
    ...
    /* === 2. Update chooser ONLY when predictors disagree === 
    if (short_pred != long_pred) {
        /* Strengthen chooser: use 3-bit (0..7) saturating counter 
        if (long_pred == taken) {
    if (*chooser_ctr < 3) (*chooser_ctr)++;  /* favor long 
      } else if (short_pred == taken) {
    if (*chooser_ctr > 0) (*chooser_ctr)--;  /* favor short */
}
    ...
    /* Update chooser if they disagree 
    if (short_pred != long_pred) {
      if (long_pred == taken) {
        if (*chooser_ctr < 3) (*chooser_ctr)++;
      } else if (short_pred == taken) {
        if (*chooser_ctr > 0) (*chooser_ctr)--;
      }
    }*/
