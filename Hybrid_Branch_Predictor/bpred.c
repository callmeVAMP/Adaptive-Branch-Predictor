...
case BPredHybrid:
{
    /* Create bimodal (2-bit) component */
    pred->dirpred.hybrid.bimod =
        bpred_dir_create(BPred2bit, /* class */
                         bimod_size, /* l1size -> table size */
                         0,          /* l2size (unused) */
                         0,          /* shift_width (unused) */
                         0);         /* xor (unused) */

    /* Create gshare component (as a 2-level predictor) */
    pred->dirpred.hybrid.gshare =
        bpred_dir_create(BPred2Level,
                         l1size,      /* L1 size (history table) */
                         l2size,      /* L2 size (pattern table) */
                         shift_width, /* history register width */
                         xor);        /* xor flag */

    /* Create chooser: use meta_size (2-bit counters) */
    pred->dirpred.hybrid.chooser =
        bpred_dir_create(BPred2bit,
                         meta_size, /* chooser table size */
                         0,
                         0,
                         0);
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  case BPredComb:
    ...
    bpred_dir_config (pred->dirpred.comb.meta, "meta", stream);
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  meta = bpred_dir_lookup (pred->dirpred.comb.meta, baddr);
